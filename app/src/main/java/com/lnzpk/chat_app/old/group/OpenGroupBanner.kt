...
class OpenGroupBanner : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.open_group_banner)
        val toolbar = findViewById<Toolbar>(R.id.openGroupBannerToolbar)
        setSupportActionBar(toolbar)
        val groupKey = intent.extras!!["groupKey"].toString()
        val groupName = intent.extras!!["groupName"].toString()
        toolbar.title = groupName
        val Pb = findViewById<ImageView>(R.id.openGroupBannerPic)
        ...
        try {
            ...
            if (picture.exists()) {
                ...
                Pb.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.background_light))
            }
        } catch (e: Exception) {
